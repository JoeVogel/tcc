...
O desafio é desenvolver um dispositivo para captação de dados relevantes ao funcionamento do trator em tempo real por meio de diversos sensores utilizando a plataforma Arduino para o processamento dos dados coletados e XBEE para envio dos dados coletados para a análise. Para a devida execução deste objetivo, alguns objetivos específicos são necessários, sendo eles:

\begin{itemize}

\item Estudar e entender mais a fundo os conceitos de Interrupção no Arduino;

\item Estudar e entender melhor o funcionamento do XBEE;

\item Compreender o conhecimento necessário sobre os sensores que serão instalados nos tratores a fim de melhor gerenciar eles com o dispositivo a ser desenvolvido;

\item Conhecer as informações básicas sobre o funcionamento do trator a fim de visualizar as situações a qual o dispositivo será submetido;
...
\item Analisar os resultados alcançados para definir melhor como ressaltar a relevância do resultado esperado.

\end{itemize}


\chapter{Arduino}\label{cap_arduino}
...
Para este dispositivo foram utilizados dois Arduinos Mega 2560 devido a quantidades de portas disponiveis para utilização.
...
\chapter{XBEE}\label{cap_xbee}

\section{Xbee Escolhi(verificar modelo)}

\section{Conclusão}

\chapter{Sensores}\label{cap_sensores}
